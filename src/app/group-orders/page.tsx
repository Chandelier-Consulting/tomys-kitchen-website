        ...
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.8fr_1.2fr]">
          ...
          <div className="grid gap-3">
            {callScript.map((question, index) => (
              <div key={question} className="grid gap-3 rounded-3xl border border-[#eadcc9] bg-white p-5 sm:grid-cols-[auto_1fr] sm:items-center">
        ...
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
          ...
          <div className="grid gap-4">
            {featuredItems.map((item) => (
              <article key={item.name} className="rounded-3xl border border-border bg-surface p-5">
        ...
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[.78fr_1.22fr]">
          <div>
            ...
            <p className="mt-5 text-base font-semibold leading-7 text-white/62">
              A few clear answers help customers move from “maybe” to “I should call now.”
            </p>
          </div>
          <div className="grid gap-3">
            {faqs.map(([question, answer]) => (
              <article key={question} className="rounded-3xl border border-white/10 bg-white/[0.06] p-5">
                <h3 className="text-xl font-black">{question}</h3>
                <p className="mt-3 text-sm font-semibold leading-6 text-white/62">{answer}</p>
              </article>
            ))}
          </div>
        </div>
